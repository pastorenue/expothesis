    ...
    pub fn from_env() -> Self {
        ...
        Self {
            ...
            postgres_url: std::env::var("DATABASE_URL")
                .unwrap_or_else(|_| "postgres://expothesis:expothesis@postgres:5432/expothesis".to_string()),
            ...
            litellm_base_url: std::env::var("LITELLM_BASE_URL").ok().filter(|value| !value.is_empty()),
            litellm_api_key: std::env::var("LITELLM_API_KEY").ok().filter(|value| !value.is_empty()),
            litellm_default_model: std::env::var("LITELLM_DEFAULT_MODEL").ok().filter(|value| !value.is_empty()),
            litellm_models: std::env::var("LITELLM_MODELS")
                ...
                .collect(),
            smtp_host: std::env::var("SMTP_HOST").ok().filter(|value| !value.is_empty()),
            smtp_user: std::env::var("SMTP_USER").ok().filter(|value| !value.is_empty()),
            smtp_pass: std::env::var("SMTP_PASS").ok().filter(|value| !value.is_empty()),
            smtp_from: std::env::var("SMTP_FROM").ok().filter(|value| !value.is_empty()),
            ...
            jwt_ttl_minutes: std::env::var("JWT_TTL_MINUTES")
                ...
                .and_then(|value| value.parse::<i64>().ok())
                .unwrap_or(60),
            default_admin_email: std::env::var("DEFAULT_ADMIN_EMAIL").ok().filter(|value| !value.is_empty()),
            default_admin_password: std::env::var("DEFAULT_ADMIN_PASSWORD").ok().filter(|value| !value.is_empty()),
        }
    }
}
